...
def test_serverless_scale_min_one_under_load_zero_when_idle() -> None:
    ...
    assert serverless_worker_scale(active=False) == {"workersMin": 0, "workersMax": 0}
...
def test_face_worker_loop_skips_claims_while_paused() -> None:
    from app.workers import face_queue as face_mod
    import inspect

    src = inspect.getsource(face_mod.FaceWorkerLoop._worker_loop)
    assert "global_indexing_is_paused" in src
