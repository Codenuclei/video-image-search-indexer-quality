...
async def _llm_topics_from_theme(
    *,
    ...
    prompt = (
        "You are an expert Instagram carousel strategist. Invent a cohesive set of TOPIC LABELS "
        "for one selected theme — a narrative arc that hangs together like an educational or "
        "storytelling carousel (hook → context → points → takeaway), not scattered tags.\n"
        "Topics are thematic angles grounded in the theme — NOT transcript quotes, NOT timestamps.\n"
        "Rules:\n"
        "- 4–8 topics that form one coherent story arc for this theme (order matters)\n"
        "- Each topic: 2–8 words, title-case or short phrase; ONE idea per topic\n"
        "- MUST be natural English (translate ideas if transcript is Hindi/Hinglish/other)\n"
        "- Ignore cue timestamps — cluster by meaning/theme so topics stay cohesive\n"
        "- No incomplete thoughts; each must stand alone as a carousel chapter title\n"
        "- Do not paste spoken dialogue as a topic\n"
        "- NEVER output near-duplicates (e.g. 'Student-First Philosophy' and "
        "'Student-Centric Decisions' are the SAME idea — keep only one best label)\n"
        "- Prefer distinct angles that advance the narrative without overlapping content\n"
