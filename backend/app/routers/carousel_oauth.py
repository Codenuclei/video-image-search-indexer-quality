"""
Carousel Studio Drive OAuth — this service is the auth server (not a sidecar).

Same URL paths as the Studio frontend already calls. Tokens are stored in this
Postgres. Credentials come from ``carousel_google()`` (CAROUSEL_GOOGLE_* then GOOGLE_*).
...
router = APIRouter(tags=["carousel-oauth"])
...
async def auth_google(
    ...
    if not creds.client_id or not creds.client_secret:
        raise HTTPException(
            status_code=501,
            detail=(
                "Google OAuth not configured. Set CAROUSEL_GOOGLE_CLIENT_ID and "
                "CAROUSEL_GOOGLE_CLIENT_SECRET (or GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET)."
            ),
        )
    params = {
...
async def get_drive_token(
    ...
    if not creds.api_key:
        raise HTTPException(
            status_code=503,
            detail=(
                "Picker API key is not set. Create a Browser API key in Google Cloud "
                "(enable Drive API + Picker API; restrict HTTP referrers to the Studio "
                "origin, e.g. http://localhost:3002/*) and set CAROUSEL_GOOGLE_API_KEY "
                "or GOOGLE_API_KEY on this service."
            ),
        )
    ...
    return {
        ...
        "appId": _google_app_id_from_client_id(creds.client_id),
    }
