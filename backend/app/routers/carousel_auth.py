"""Carousel Studio GIS login — verify Google ID token against this service's client.

Copied from Drive-search ``app_auth`` so this backend is the auth server, not a
sidecar. Tokens are checked with this process's Google client id; ``app_admins``
rows live in this Postgres.
"""
...
router = APIRouter(tags=["carousel-auth"])
...
def _verify_google_id_token(credential: str, settings: Settings) -> dict[str, Any]:
    ...
    if not client_id:
        raise HTTPException(
            status_code=503,
            detail=(
                "Google client id is not configured. Set CAROUSEL_GOOGLE_CLIENT_ID "
                "or GOOGLE_CLIENT_ID on this service."
            ),
        )
    try:
...
async def exchange_google_id_token(
    ...
    """Verify GIS credential; return email + whether it is in this DB's ``app_admins``."""
